...
describe 'Check custom fields create and update' do
  ...
  it 'should create a new select box custom field' do
    ...
    client.delete("custom_fields/#{new_cf_id}.json")
  ...
  it 'should create and update new contact with custom fields the documented way' do
    ...
    response = client.post('custom_fields.json', second_new_cf)
    ...
    client.delete("custom_fields/#{second_cf_id}.json")
  end
  ...
  it 'should create and update new contact with custom fields the undocumented way' do
    ...
    response = client.put("contacts/#{new_contact_id}.json", updated_contact_details)
    got_deets = client.get_contact_details(new_contact_id)['data']['contact']
  ...
  it 'should get contacts with a certain custom field value' do
    ...
    B_id = client.post('contacts.json', B_contact_details)['data']['contact']['id']
    A_id = client.post('contacts.json', A_contact_details)['data']['contact']['id']
    no_id = client.post('contacts.json', no_contact_details)['data']['contact']['id']

    # call
    response = client.get("contacts.json?custom_field_id=#{cf_id}&custom_field_value=B")
    expect(response['data']['contacts'].size).to be 1
    expect(response['data']['contacts'][0]['contact']['first_name']).to eq 'Right'

    client.delete("contacts/#{B_id}.json")
    client.delete("contacts/#{A_id}.json")
    client.delete("contacts/#{no_id}.json")
  end
