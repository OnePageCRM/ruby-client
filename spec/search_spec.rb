...
describe 'Test Actions' do
  ...
  before(:each) do
    ...
    new_contact = client.create_contact(new_contact_details_1)
    ...
    new_contact = client.create_contact(new_contact_details_2)
    ...
    new_contact = client.create_contact(new_contact_details_3)
    @new_contact_id_3 = new_contact['contact']['id']
  end

  after(:each) do
    client.delete("contacts/#{@new_contact_id_1}.json")
    client.delete("contacts/#{@new_contact_id_2}.json")
    client.delete("contacts/#{@new_contact_id_3}.json")
  end

  it 'should find three contacts with string Search in it' do
    response = client.get('contacts.json?search=search')
    expect(response['data']['contacts'].count).to be 3
  end

  it 'should find one contact with email address search2@email.com' do
    response = client.get('contacts.json?email=search2@email.com')
    expect(response['data']['contacts'].count).to be 1
  end

  it 'should find two contacts with website rossespointguesthouse.com' do
    response = client.get('contacts.json?url=rossespointguesthouse.com')
    expect(response['data']['contacts'].count).to be 2
  end

  it 'should find one starred contact' do
    response = client.get('contacts.json?starred=TRUE')
    expect(response['data']['contacts'].count).to be 1
    response = client.get('contacts.json?starred=true')
    expect(response['data']['contacts'].count).to be 1
    response = client.get('contacts.json?starred=1')
    expect(response['data']['contacts'].count).to be 1
  end

  it 'should find contacts with matching tags' do
    response = client.get('contacts.json?tag=a')
    expect(response['data']['contacts'][0]['contact']['id']).to eq @new_contact_id_2.to_s
    response = client.get('contacts.json?tag=b')
    expect(response['data']['contacts'].count).to be 2
    response = client.get('contacts.json?tag=c')
    expect(response['data']['contacts'][0]['contact']['id']).to eq @new_contact_id_3.to_s
  end

  it 'should find contacts with actions' do
    response = client.get('contacts.json?has_actions=true')
    expect(response['data']['contacts'].count).to be 0
    response = client.post('actions.json', { 'assignee_id' => client.return_uid,
                                             'contact_id' => @new_contact_id_3,
                                             'text' => 'HASACTIONS',
                                             'status' => 'asap' })
    response = client.get('contacts.json?has_actions=true')
    expect(response['data']['contacts'].count).to be 1
    response = client.get('contacts.json?has_actions_for_me=true')
    expect(response['data']['contacts'].count).to be 1
  end
